...
```{r}
library(readr)
bank_data <- read_csv("C:/Users/muska/Downloads/bank.csv.xls")
attach(bank_data)
str(bank_data)
...
d.all <- data.frame(y,sqrt(rowSums(matabsdev.all[,-1]^2)))
d.all
colnames(d.all)[2] <- "dij"
d.all
head(d.all)
with(d.all, t.test(dij[y], dij[y=="no"],var.equal=TRUE, alternative="less"))
sprintf("d-values for yes: Mean = %2.3f, Variance = %2.3f",mean(d.all$dij[y=="yes"]),var(d.all$dij[y=="yes"]))
sprintf("d-values for no: Mean = %2.3f, Variance = %2.3f",mean(d.all$dij[y=="no"]),var(d.all$dij[y=="no"]))
# Hotelling Test
...
fviz_pca_var(bank_pca,col.var = "cos2",
             gradient.cols = c("#FFCC00", "#CC9933", "#660033", "#330033"),
             repel = TRUE)
fviz_pca_ind(bank_pca, col.ind = "cos2", 
                  gradient.cols = c("#FFCC00", "#CC9933", "#660033", "#330033"), 
                  repel = TRUE)
biplot(bank_pca)
autoplot(bank_pca,
...
fviz_pca_ind(res.pca, geom.ind = "point", col.ind = bank_data$y, 
             palette = c("#00AFBB", "#E7B800", "#FC4E07"),
...
fviz_pca_ind(res.pca,
             ...
             habillage = bank_data$y, # color by groups
             addEllipses = TRUE, # Concentration ellipses
             palette = "jco"
             )
...
fviz_pca_ind(res.pca,
             ...
             group.ind = bank_data$y, # color by groups
             legend.title = "Groups",
             mean.point = FALSE)
fviz_pca_ind(res.pca,
             geom.ind = "point", # show points only (but not "text")
...
ind.p <- fviz_pca_ind(res.pca, geom = "point", col.ind = bank_data$y)
ggpubr::ggpar(ind.p,
              title = "Principal Component Analysis",
              subtitle = "Iris data set",
              caption = "Source: factoextra",
              xlab = "PC1", ylab = "PC2",
              legend.title = "y", legend.position = "top",
              ggtheme = theme_gray(), palette = "jco"
              )

fviz_pca_biplot(res.pca, repel = TRUE,col.ind = bank_data$y,
                col.var = "#2E9FDF", # Variables color
                )

fviz_pca_biplot(res.pca, 
                col.ind = bank_data$y, palette = "jco", 
...
fviz_pca_biplot(res.pca, 
                ...
                pointshape = 21,
                pointsize = 2.5,
                fill.ind = bank_data$y,
                col.ind = "black",
                # Color variable by groups
                legend.title = list(fill = "y", color = "Clusters"),
...
fviz_pca_biplot(res.pca, 
                # Individuals
                geom.ind = "point",
                fill.ind = bank_data$y, col.ind = "black",
                ...
                legend.title = list(fill = "y", color = "Contrib",
                                    alpha = "Contrib")
                )
...
xyplot(age~duration)
xyplot(age~duration | day)
xyplot(age~duration, groups=day)
xyplot(age~duration | day + duration)
xyplot(age~duration | day , groups=duration, auto.key=list(space="right"))

xyplot(age~duration, col="steelblue", pch=3, main="BANK Data", xlab="age", 
       ylab="duration", xlim=c(0,3), scales=list(tick.number=10))
...
lastplot <- ggplot(bank_data, aes(x=age,y=duration)) + xlim(0,3) + geom_point(aes(color=cut)) + stat_smooth() + 
  labs(x="age", y="duration", title="bank Data") 
